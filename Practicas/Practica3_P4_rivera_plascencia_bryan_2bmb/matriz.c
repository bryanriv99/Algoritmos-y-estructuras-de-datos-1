...
MATRIZ crearMatriz(int filas, int columans){
    ...
    m->mtz = (int**) malloc(sizeof(int*)*filas);
    ...
    for (int i = 0; i < filas; i++){
        m->mtz[i] = (int*) malloc(sizeof(int)*columans);
    }
...
MATRIZ transpuesta(MATRIZ m1){
    ...
    for (int i = 0; i < m1->filas; i++){
            for (int j = 0; j < m1->columas; j++){
                mResultante->mtz[j][i] = m1->mtz[i][j];
            }
        }
    return mResultante;
}
...
MATRIZ multiplicaMatriz(MATRIZ m1, MATRIZ m2){
    if((m1->columas == m2->columas) && (m1->filas==m2->filas)){
        ...
        for (int i = 0; i < m1->filas; i++){
            for (int j = 0; j < m1->columas; j++){
                mResultante->mtz[i][j] = m1->mtz[i][j] * m2->mtz[i][j];
            }
        }
        return mResultante;
